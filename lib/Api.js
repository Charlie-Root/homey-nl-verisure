...
var timer = ms => new Promise( res => setTimeout(res, ms));
	...
	const getDoorWindow =() => {
		...
		if(data !== null) {
			setDoorWindow(data["response"]["installationOverview"][0]["doorWindow"][0]["doorWindowDevice"]);
		}
	}
	...
	const getClimateStatus =() => {
		...
		var data = Homey.ManagerSettings.get('apiData');
		if(data !== null) {
			setClimateStatus(data["response"]["installationOverview"][0]["climateValues"][0]);
		}
	}
		...
		console.log("setLockState");
			...
			if (Homey.ManagerSettings.get('username') != null && Homey.ManagerSettings.get('keycode') != null) {      
				...
				var opt = {
				  ...
				  sendRequest(opt).catch(logger).catch(logger);
